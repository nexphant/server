...
CDEF;
    ...
    public function __construct(?string $library = null)
    {
        ...
        $library ??= getenv('NEXPH_NATIVE_LIB') ?: '';
    ...
    public function findHeaderEnd(string $buffer, int $offset = 0): int|false
    {
        ...
        $pos = $this->ffi->nexph_find_header_end($buffer, strlen($buffer), max(0, $offset));
    ...
    public function hasConnectionClose(string $buffer): bool
    {
        ...
        return $this->ffi->nexph_has_connection_close($buffer, strlen($buffer)) === 1;
    }
